...
async def process_image(message: Message, image_bytes: bytes, state: FSMContext):
    ...
    try:
        ...
        # Снимок результата для будущего сравнения/динамики (не влияет на выдачу).
        try:
            ...
            save_result_snapshot(
                ...
                tier=(analysis.get("tier") or {}).get("abbr"),
                top_percent=analysis.get("top_percent"),
                weakest_metric=_weakest,
                product=product,
            )
        except Exception:
            logger.exception("save_result_snapshot failed (non-blocking)")
