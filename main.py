...
from counters import (
    ...
    enqueue_return_tasks, get_due_return_tasks, mark_return_task_sent,
    cleanup_old_return_tasks, was_return_sent_today,
    record_event, track_user_seen, mark_user_blocked, get_admin_stats,
    get_today_stats, claim_admin_notification,
)
    ...
    if event == "payment.succeeded" or status == "succeeded":
        ...
        if verified:
            ...
            if order:
                ...
                notify_admins_payment_sync(order)
...
_PRODUCT_TITLES = {
    PRODUCT_FACE_REPORT: "Обычный разбор",
    PRODUCT_PREMIUM_PLAN: "Premium Plan",
}


def _build_payment_admin_text(order: dict) -> str:
    title = _PRODUCT_TITLES.get(order.get("product"), order.get("product") or "—")
    ...
    try:
        ...
        today_block = (
            f"\n\n<b>Сегодня:</b>\n"
            f"Обычный разбор: {t['paid_face']}\n"
            f"Premium: {t['paid_premium']}\n"
            f"Выручка: {t['revenue']}"
        )
    except Exception:
        today_block = ""
...
def notify_admins_payment_sync(order: dict):
    """Синхронное уведомление админов (для webhook). Дедуп + безопасно."""
    try:
        if not order or not ADMIN_IDS:
            return
        order_id = order.get("order_id")
        if not order_id or not claim_admin_notification(order_id):
            return
        text = _build_payment_admin_text(order)
        for admin_id in ADMIN_IDS:
            try:
                send_bot_message_sync(admin_id, text)
            except Exception:
                logger.exception("notify_admins_payment_sync: send failed for %s", admin_id)
    except Exception:
        logger.exception("notify_admins_payment_sync failed (non-blocking)")


async def notify_admins_payment(order: dict):
    try:
        if not order or not ADMIN_IDS:
            return
        order_id = order.get("order_id")
        if not order_id:
            return
        if not claim_admin_notification(order_id):
            return
        text = _build_payment_admin_text(order)
        for admin_id in ADMIN_IDS:
            try:
                ...
                logger.exception("notify_admins_payment: send failed for admin %s", admin_id)
    except Exception:
        logger.exception("notify_admins_payment failed (non-blocking)")
...
async def cmd_start_entry(message: Message, command: CommandObject, state: FSMContext):
    ...
    track_user_seen(message.from_user.id)  # дата первого появления для статистики
...
def _fmt_admin_stats(s: dict) -> str:
    u = s["users"]; f = s["funnel"]; m = s["money"]
    p = s["product"]; e = s["errors"]; r = s["referrals"]
    tiers = " · ".join(f"{k}:{v}" for k, v in sorted(p["tiers"].items(),
                       key=lambda kv: -kv[1])) or "—"
    return (
        "📊 <b>Админ-статистика Heim Face</b>\n\n"
        "<b>👤 Пользователи</b>\n"
        f"Всего: {u['total']} · Активных: {u['active']} · Заблок.: {u['blocked']}\n"
        f"Новые: сегодня {u['new_today']} · 7д {u['new_7']} · 30д {u['new_30']}\n\n"
        "<b>🛒 Воронка</b>\n"
        f"Фото: {f['photos']} · Разборов: {f['analyses']}\n"
        f"Оплат: обычный {f['paid_face']} · Premium {f['paid_premium']}\n"
        f"CR start→фото: {f['cr_start_photo']}%\n"
        f"CR фото→разбор: {f['cr_photo_analysis']}%\n"
        f"CR фото→оплата: {f['cr_photo_paid']}%\n"
        f"CR обычный→Premium: {f['cr_face_premium']}%\n\n"
        "<b>💰 Деньги</b>\n"
        f"Всего: {m['total']} · Сегодня: {m['today']}\n"
        f"7д: {m['d7']} · 30д: {m['d30']}\n"
        f"Оплат: обычный {m['paid_face']} · Premium {m['paid_premium']}\n\n"
        "<b>🎯 Продукт</b>\n"
        f"Средний score: {p['avg_score']} · Отзывы: {p['rating_avg']} ({p['rating_count']})\n"
        f"Tier: {tiers}\n\n"
        "<b>⚠️ Ошибки</b>\n"
        f"Анализ: {e['analysis']} · Оплата: {e['payment']} · "
        f"PDF: {e['pdf']} · Отправка: {e['send']}\n\n"
        "<b>🔗 Рефералка</b>\n"
        f"Приглашено: {r['invited']} · Активных: {r['active']} · Бонусов: {r['bonus']}"
    )
...
async def cmd_admin_stats(message: Message):
    ...
    try:
        ...
        await message.answer(_fmt_admin_stats(stats), parse_mode="HTML")
    except Exception:
        logger.exception("admin_stats failed")
        await message.answer("Не удалось собрать статистику.")


def _fmt_admin_today(t: dict) -> str:
    score = f"{t['avg_score']} ({t['scores_count']})" if t["avg_score"] is not None else "—"
    return (
        "📅 <b>Heim Face — сегодня</b>\n\n"
        f"👤 Новых: {t['new_users']} · Заблок.: {t['blocked']}\n"
        f"📸 Фото: {t['photos']} · Разборов: {t['analyses']}\n"
        f"🛒 Оплат: обычный {t['paid_face']} · Premium {t['paid_premium']}\n"
        f"💰 Выручка: {t['revenue']}\n"
        f"🎯 Средний score: {score}\n"
        f"⚠️ Ошибки: анализ {t['err_analysis']} · PDF {t['err_pdf']} · отправка {t['err_send']}"
    )


@dp.message(Command("admin_today"))
async def cmd_admin_today(message: Message):
    if not is_admin(message.from_user.id):
        await message.answer("Недоступно.")
        return
    try:
        await message.answer(_fmt_admin_today(get_today_stats()), parse_mode="HTML")
    except Exception:
        logger.exception("admin_today failed")
        await message.answer("Не удалось собрать статистику.")
...
# ================== POST-PURCHASE RETURN (7/30/60) ==================
RETURN_CHECK_INTERVAL_SECONDS = 3 * 60 * 60  # каждые 3 часа

_RETURN_MESSAGES = {
    PRODUCT_FACE_REPORT: {
        "day_7": "Прошла неделя после разбора. Если начали внедрять рекомендации — можно уже увидеть первые изменения в подаче, свете и уходе.",
        "day_30": "Прошёл месяц после вашего разбора. Самое время сделать повторное фото и проверить динамику score/tier.",
        "day_60": "Прошло 60 дней. Лицо, подача и привычки могли измениться — обновите разбор и сравните результат.",
    },
    PRODUCT_PREMIUM_PLAN: {
        "day_7": "Прошла неделя Premium-плана. Проверьте, какие шаги уже внедрили: брови, фото-подача, депафф, grooming.",
        "day_30": "30 дней — идеальная точка для повторного замера. Сделайте новое фото и сравните score/tier с прошлым результатом.",
        "day_60": "Прошло 60 дней после Premium. Можно обновить разбор и увидеть, что изменилось по метрикам.",
    },
}


def _return_message_text(product: str, ttype: str) -> str | None:
    table = _RETURN_MESSAGES.get(product) or _RETURN_MESSAGES.get(PRODUCT_FACE_REPORT)
    return table.get(ttype) if table else None


def _return_keyboard(product: str) -> InlineKeyboardMarkup:
    # Ведёт в существующий flow выбора тарифа, без ручной оплаты.
    target = product if product in {PRODUCT_FACE_REPORT, PRODUCT_PREMIUM_PLAN} else PRODUCT_FACE_REPORT
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔁 Сделать повторный разбор", callback_data=f"buy:{target}")],
    ])


async def _process_due_return_tasks():
    """Один проход по очереди возврата: отправляет НЕ более одной задачи на
    пользователя за проход и не более одной в день. Помечает sent=true."""
    try:
        due = get_due_return_tasks()
    except Exception:
        logger.exception("get_due_return_tasks failed")
        return
    seen_users = set()
    for task in due:
        uid = task.get("user_id")
        if uid in seen_users:
            continue  # не больше одной за проход на пользователя
        try:
            if was_return_sent_today(uid):
                seen_users.add(uid)
                continue  # антиспам: не больше одного в день
            text = _return_message_text(task.get("product"), task.get("type"))
            if not text:
                mark_return_task_sent(task["id"])  # неизвестный тип — закрываем
                continue
            await bot.send_message(uid, text, reply_markup=_return_keyboard(task.get("product")))
            mark_return_task_sent(task["id"])
            seen_users.add(uid)
        except TelegramForbiddenError:
            logger.warning("Return: пользователь заблокировал бота: %s", uid)
            mark_user_blocked(uid)
            mark_return_task_sent(task["id"])
            seen_users.add(uid)
        except Exception:
            # Блокировка ботом / ошибка отправки — помечаем, чтобы не долбить.
            logger.exception("return task send failed for user %s", uid)
            try:
                mark_return_task_sent(task["id"])
            except Exception:
                pass
...
async def _return_tasks_worker():
    """Фоновый воркер: периодически обрабатывает очередь возврата.
    Не блокирует бота, ошибки не роняют polling."""
    while True:
        try:
            await _process_due_return_tasks()
            cleanup_old_return_tasks()
        except Exception:
            logger.exception("return tasks worker iteration failed")
        await asyncio.sleep(RETURN_CHECK_INTERVAL_SECONDS)


def _build_dynamics_message(prev: dict, new_score, new_tier, new_product=None) -> str | None:
    """Строит сообщение динамики «было → стало». Возвращает None, если
    показывать динамику не следует (недостаточно данных, тот же день,
    апгрейд Premium на том же фото, изменение меньше порога)."""
    try:
    ...
    # Не показывать, если прошлый снимок сделан в тот же календарный день
    # (это апгрейд Premium / повторная отправка того же фото, а не новый разбор).
    prev_created = str(prev.get("created_at", ""))[:10]
    today = datetime.now().isoformat()[:10]
    if prev_created and prev_created == today:
        return None

    delta = round(new_s - old_s, 2)

    # Минимальный порог: ниже 0.10 по модулю — молчим (в т.ч. delta == 0,
    # тот же score, апгрейд face_report -> premium_plan на том же фото).
    if abs(delta) < 0.10:
        return None
...
async def check_payment_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if order.get("provider") == PROVIDER_YOOKASSA and verify_yookassa_payment(payment_id, order_id):
        order = mark_order_paid(order_id, payment_id)
        credit_referral_for_order(order)
        await notify_admins_payment(order)
        await state.set_state(AnalysisStates.waiting_for_gender)
...
async def _send_pdf_logged(message: Message, path: str, filename: str, reply_markup=None) -> bool:
    ...
    try:
        ...
        record_event("error_send")
        await message.answer(TELEGRAM_TIMEOUT_MSG, reply_markup=main_keyboard)
        return False
    except TelegramForbiddenError:
        logger.warning("Пользователь заблокировал бота: %s", message.chat.id)
        record_event("error_send")
        mark_user_blocked(message.chat.id)
        return False
    except Exception:
...
async def process_image(message: Message, image_bytes: bytes, state: FSMContext):
    ...
    if error:
        record_event("error_analysis")
        await message.answer(f"{error}\n\nПопробуйте другое фото.",
                             reply_markup=cancel_keyboard)
        return
    record_event("analysis_success")
    ...
    try:
        ...
        try:
            ...
            if _saved and _prev:
                _msg = _build_dynamics_message(_prev, _new_score, _new_tier, new_product=product)
                if _msg:
                    await message.answer(_msg, parse_mode="HTML")
            # Очередь возврата 7/30/60 дней (персистентная, не asyncio.sleep).
            enqueue_return_tasks(message.from_user.id, product)
        except Exception:
...
async def main():
    ...
    asyncio.create_task(_return_tasks_worker())  # фоновый возврат 7/30/60 дней
