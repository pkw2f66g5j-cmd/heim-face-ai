...
DEMO_FACE_PATH = os.path.join(BASE_DIR, "assets", "demo", "demo_face_report.pdf")
DEMO_PREMIUM_PATH = os.path.join(BASE_DIR, "assets", "demo", "demo_premium_plan.pdf")
...
def product_description(product: str) -> str:
    if product == PRODUCT_PREMIUM_PLAN:
        return (
            ...
            "Полный разбор и личный план: не только цифры, но и конкретные шаги "
            "по улучшению внешности.\n\n"
            "<b>В пакете:</b>\n"
            "✅ Полный математический разбор лица (23 страницы)\n"
            "✅ Персональный луксмаксинг-план\n"
            "✅ Приоритеты улучшений по силе влияния\n"
            ...
            "🎁 <b>Бонус:</b> гайд по снижению отёчности\n\n"
            "Доступна оплата картой, СБП через ЮKassa или Telegram Stars.\n\n"
...
def _demo_files_for(product: str):
    ...
    if product == PRODUCT_PREMIUM_PLAN:
        return [
            ...
            (DEMO_PREMIUM_PATH, "Пример · Premium Plan.pdf"),
        ]
    return [(DEMO_FACE_PATH, "Пример · Разбор лица.pdf")]


async def _send_demo(message: Message, product: str, closing: str | None = None) -> bool:
    """Отправляет демо-PDF тарифа. Возвращает False, если файлов нет.
    Не создаёт заказ и не меняет FSM-состояние."""
    available = [(p, name) for p, name in _demo_files_for(product) if os.path.exists(p)]
    if not available:
        return False
...
async def demo_callback(callback: CallbackQuery):
    ...
    ok = await _send_demo(
        ...
        closing="Это пример формата. Ваш отчёт строится по вашему фото.",
    )
    if not ok:
        await callback.answer("Пример отчёта временно недоступен. Попробуйте позже.", show_alert=True)
