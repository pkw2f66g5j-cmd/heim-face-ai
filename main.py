...
DEMO_FACE_PATH = os.path.join(BASE_DIR, "assets", "demo_face_report.pdf")
DEMO_PREMIUM_PATH = os.path.join(BASE_DIR, "assets", "demo_premium_plan.pdf")
...
async def demo_callback(callback: CallbackQuery):
    ...
    пользователь остаётся на экране оплаты."""
    product = callback.data.split(":", 1)[1]

    # Для Premium показываем оба примера, для разбора — только базовый.
    if product == PRODUCT_PREMIUM_PLAN:
        files = [
        ...
        files = [(DEMO_FACE_PATH, "Пример · Разбор лица.pdf")]

    available = [(p, name) for p, name in files if os.path.exists(p)]
    ...
    await callback.message.answer(
        "Это пример формата. Ваш отчёт строится по вашему фото.\n\n"
        "Выберите способ оплаты выше, чтобы получить персональный разбор.",
    )
