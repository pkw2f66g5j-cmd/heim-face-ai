...
def _default_data() -> dict:
    return {
        ...
        "user_seen": {},       # uid -> ISO date первого появления
        "blocked_users": [],   # uid пользователей, заблокировавших бота
        "daily": {},           # "YYYY-MM-DD" -> {события + blocked за день}
    }
...
def _normalize(data: dict) -> dict:
    ...
    if not isinstance(data.get("ratings"), dict):
        data["ratings"] = {"count": 0, "sum": 0}

    if not isinstance(data.get("events"), dict):
        data["events"] = {}
    for _ev in ("photo_uploaded", "analysis_success", "error_analysis",
                "error_payment", "error_pdf", "error_send"):
        if not isinstance(data["events"].get(_ev), int):
            data["events"][_ev] = 0

    if not isinstance(data.get("user_seen"), dict):
        data["user_seen"] = {}
    if not isinstance(data.get("blocked_users"), list):
        data["blocked_users"] = []
...
def claim_admin_notification(order_id: str) -> bool:
    """Атомарно «занимает» право на отправку админ-уведомления по заказу.
    Возвращает True только при первом вызове для данного order_id
    (защита от дублей при повторных webhook/проверках). Иначе False."""
    try:
        c = _load()
        order = c["orders"].get(order_id)
        if not order:
            return False
        if order.get("admin_notified_at"):
            return False
        order["admin_notified_at"] = datetime.now().isoformat(timespec="seconds")
        _save(c)
        return True
    except Exception:
        return False
...
# ================== ADMIN STATS ==================
_EVENT_KEYS = {"photo_uploaded", "analysis_success", "error_analysis",
               "error_payment", "error_pdf", "error_send"}


def _today_bucket(c) -> dict:
    """Возвращает (создавая при необходимости) словарь счётчиков за сегодня."""
    today = date.today().isoformat()
    bucket = c["daily"].get(today)
    if not isinstance(bucket, dict):
        bucket = {}
        c["daily"][today] = bucket
    return bucket


def record_event(event: str, n: int = 1):
    """Инкремент событийного счётчика (кумулятивный + дневной). Не падает."""
    try:
        if event not in _EVENT_KEYS:
            return
        c = _load()
        c["events"][event] = int(c["events"].get(event, 0)) + n
        bucket = _today_bucket(c)
        bucket[event] = int(bucket.get(event, 0)) + n
        _save(c)
    except Exception:
        pass


def track_user_seen(user_id: int):
    """Фиксирует дату первого появления пользователя (для new-today/7d/30d)."""
    try:
        c = _load()
        uid = str(user_id)
        if uid not in c["user_seen"]:
            c["user_seen"][uid] = date.today().isoformat()
            _save(c)
    except Exception:
        pass
...
def mark_user_blocked(user_id: int):
    """Помечает, что пользователь заблокировал бота."""
    try:
        c = _load()
        uid = str(user_id)
        if uid not in c["blocked_users"]:
            c["blocked_users"].append(uid)
            bucket = _today_bucket(c)
...
def unmark_user_blocked(user_id: int):
    """Снимает отметку блокировки (если пользователь вернулся)."""
    try:
        c = _load()
        uid = str(user_id)
        if uid in c["blocked_users"]:
            c["blocked_users"].remove(uid)
            _save(c)
    except Exception:
        pass
...
def _pct(part, whole) -> float:
    try:
        if not whole:
            return 0.0
        return round(min(part / whole * 100, 100.0), 1)
    except Exception:
        return 0.0


def get_admin_stats() -> dict:
    """Собирает полную админ-статистику из существующих данных. Не падает
    на старом counters.json (отсутствующие поля → 0)."""
    c = _load()
    today = date.today()
    ...
    # --- Пользователи ---
    seen = c.get("user_seen", {})
    total_users = len(seen) or len(c.get("users", {}))
    blocked = c.get("blocked_users", [])
    blocked_n = len(blocked)
    active_n = max(0, total_users - blocked_n)
    new_today = new_7 = new_30 = 0
    ...
    # --- Воронка ---
    ev = c.get("events", {})
    photos = int(ev.get("photo_uploaded", 0))
    analyses = int(ev.get("analysis_success", 0))
    paid_face = int(c.get("paid_face_report_count", 0))
    paid_prem = int(c.get("paid_premium_plan_count", 0))
    paid_total = paid_face + paid_prem

    funnel = {
        "photos": photos,
        "analyses": analyses,
        "paid_face": paid_face,
        "paid_premium": paid_prem,
        "cr_start_photo": _pct(photos, total_users),
        "cr_photo_analysis": _pct(analyses, photos),
        "cr_photo_paid": _pct(paid_total, photos),
        "cr_face_premium": _pct(paid_prem, paid_face + paid_prem),
    }

    # --- Деньги (из оплаченных заказов) ---
    rev_total = rev_today = rev_7 = rev_30 = 0
    for o in c.get("orders", {}).values():
        if not isinstance(o, dict):
            continue
        if o.get("status") not in (ORDER_PAID, ORDER_CONSUMED):
            continue
        amt = o.get("amount") or 0
        rev_total += amt
        pd = _parse(o.get("paid_date"))
        if pd:
            ago = _days_ago(pd)
            if ago == 0:
                rev_today += amt
            ...
            if ago <= 30:
                rev_30 += amt
    money = {
        "total": rev_total, "today": rev_today, "d7": rev_7, "d30": rev_30,
        "paid_face": paid_face, "paid_premium": paid_prem,
    }

    # --- Продукт (из снимков результатов + рейтингов) ---
    scores, tiers = [], {}
    for hist in c.get("user_results", {}).values():
    ...
    r = c.get("ratings", {})
    rcount = int(r.get("count", 0))
    ravg = round(r.get("sum", 0) / rcount, 2) if rcount else 0.0
    product = {
    ...
    # --- Ошибки ---
    errors = {
        "analysis": int(ev.get("error_analysis", 0)),
        "payment": int(ev.get("error_payment", 0)),
        "pdf": int(ev.get("error_pdf", 0)),
        "send": int(ev.get("error_send", 0)),
    }
    ...
    for rec in refs.values():
        if not isinstance(rec, dict):
            continue
        credited = rec.get("credited_referrals", []) or []
        invited += len(credited)
        if credited:
            active_refs += 1
        if rec.get("free_premium_count", 0) > 0 or rec.get("paid_referrals_count", 0) >= 3:
            bonus_received += 1
    referrals = {"invited": invited, "active": active_refs, "bonus": bonus_received}

    return {
        "users": {
            "total": total_users, "active": active_n, "blocked": blocked_n,
            "new_today": new_today, "new_7": new_7, "new_30": new_30,
        },
        "funnel": funnel,
        "money": money,
        "product": product,
...
def get_today_stats() -> dict:
    """Срез метрик строго за сегодня. Использует daily-бакет (события/blocked)
    и датированные данные (orders.paid_date, user_seen, user_results.created_at)."""
    c = _load()
    today = date.today().isoformat()
    bucket = c.get("daily", {}).get(today, {}) or {}

    # Новые пользователи сегодня
    new_users = sum(1 for ds in c.get("user_seen", {}).values()
                    if str(ds)[:10] == today)

    # Оплаты и выручка сегодня (из заказов по paid_date)
    paid_face = paid_prem = revenue = 0
    for o in c.get("orders", {}).values():
        if not isinstance(o, dict):
            continue
        if o.get("status") not in (ORDER_PAID, ORDER_CONSUMED):
            continue
        if str(o.get("paid_date"))[:10] != today:
            continue
        amt = o.get("amount") or 0
        revenue += amt
        if o.get("product") == PRODUCT_PREMIUM_PLAN:
            paid_prem += 1
        elif o.get("product") == PRODUCT_FACE_REPORT:
            paid_face += 1

    # Средний score за сегодня (снимки, созданные сегодня)
    scores = []
    for hist in c.get("user_results", {}).values():
        if not isinstance(hist, list):
            continue
        for snap in hist:
            if isinstance(snap, dict) and str(snap.get("created_at"))[:10] == today:
                s = snap.get("score")
                if isinstance(s, (int, float)):
                    scores.append(float(s))
    ...
    return {
        "new_users": new_users,
        "photos": int(bucket.get("photo_uploaded", 0)),
        "analyses": int(bucket.get("analysis_success", 0)),
        "paid_face": paid_face,
        "paid_premium": paid_prem,
        "revenue": revenue,
        "avg_score": avg_score,            # None, если за сегодня нет данных
        "scores_count": len(scores),
        "err_analysis": int(bucket.get("error_analysis", 0)),
        "err_pdf": int(bucket.get("error_pdf", 0)),
        "err_send": int(bucket.get("error_send", 0)),
