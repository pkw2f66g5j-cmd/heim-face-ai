...
def create_share_card(data: dict, output_path: str) -> str:
    ...
    # Top-N% бейдж (статусная плашка) — только если данные есть.
    top_pct = data.get("top_percent")
    if top_pct is not None:
        try:
            badge_txt = f"ТОП {int(top_pct)}% ПО ГЕОМЕТРИИ"
            f_badge = _load(bold_path, 28)
            ...
            bx0 = (CARD_W - bw) / 2
            by0 = 224
            # пилюля: тёмный фон + золотая обводка
            draw.rounded_rectangle([bx0, by0, bx0 + bw, by0 + bh], radius=bh / 2,
                                   fill=BG_TOP, outline=GOLD, width=2)
            draw.text((bx0 + pad_x, by0 + pad_y - bb[1]), badge_txt, font=f_badge, fill=GOLD)
        except Exception:
            pass
