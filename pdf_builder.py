...
def _lux_presentation_page(c, w, h, data, gender, page_num, total_pages):
    ...
    items = list(_photo_plan(data))[:5]
    intro = (
        f"Как подавать лицо сильнее в кадре. Цель — показать «{_dominant_strength(data)['name']}» "
        "и не выводить в первый план главный лимитер. Ракурс и свет привязаны к вашим метрикам."
    )
    _lux_section_page(c, w, h, page_num, total_pages, "Presentation",
                      "Как подавать лицо сильнее в кадре", intro, items, body_max_lines=3)
...
def _lux_daily_base_page(c, w, h, data, gender, page_num, total_pages):
    ...
    items = [
        ("Зона фокуса депаффа", f"Отёчность сильнее всего бьёт по зоне: {focus}. Контроль здесь не даёт лимитеру «{limiter['name']}» считываться заметнее."),
...
def _lux_hair_page(c, w, h, data, gender, page_num, total_pages):
    ...
    _lux_section_page(c, w, h, page_num, total_pages, "Hair", "Причёска", intro, items,
                      personal=_personal_line(data, FRAME_METRICS))
...
def _lux_brows_page(c, w, h, data, gender, page_num, total_pages):
    ...
    _lux_section_page(c, w, h, page_num, total_pages, "Brows", "Брови", intro, items,
                      personal=_personal_line(data, EYE_METRICS))
