...
def _lux_section_page(c, w, h, page_num, total_pages, kicker, title, intro, items, body_max_lines=3):
    _lux_header(c, w, h, kicker, title, page_num, total_pages)
    y = h - 170
    y = _lux_text(c, intro, 58, y, max_width=w - 116, lh=15, size=10.4, color=LUX_TEXT_SOFT, min_y=62, max_lines=3)
    y -= 20
    # При большом числе пунктов ограничиваем тело, чтобы не было переполнения.
    if body_max_lines is None:
        body_max_lines = 2 if len(items) >= 7 else 3
    _lux_bullets(c, items, 64, y, max_chars=88, max_width=w - 144, min_y=62, body_max_lines=body_max_lines)
